...
func BuildApp(db *gorm.DB, redis2 *redis.Client, minio2 *minio.Client, mqConn *amqp.Connection) *gin.Engine {
	...
	bloomRepo := repository.NewBloomRepo(redis2, redisGuard)
	storage := oss.NewStorage(minio2)
	uploadVerificationPublisher := mq.NewUploadVerificationPublisher(mqConn)
	bloomRepairPublisher := mq.NewBloomRepairPublisher(mqConn)
	fileService := service.NewFileService(fileRepo, fileCacheRepo, uploadCacheRepo, rateLimiter, userRepo, userCacheRepo, bloomRepo, storage, db, cacheInvalidationPublisher, uploadVerificationPublisher, bloomRepairPublisher)
	...
	shareService := service.NewShareService(shareRepo, shareCacheRepo, fileRepo, rateLimiter, bloomRepo, storage, bloomRepairPublisher)
