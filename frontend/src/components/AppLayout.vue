  ...
  <div class="flex min-h-screen bg-background">
    <aside class="flex w-56 shrink-0 flex-col border-r border-border bg-card p-3">
      <div class="mb-3 flex items-center gap-2 rounded-md border border-border bg-white px-3 py-3 shadow-sm">
        <Cloud class="h-5 w-5 text-foreground" />
        <span class="text-[20px] font-semibold leading-[1.4] text-foreground">WeDrive</span>
      </div>
      ...
      <nav class="space-y-1">
        <RouterLink
          ...
          :to="item.path"
          :class="[
            'flex items-center gap-2 rounded-md border px-3 py-2 text-[14px] leading-[1.6] transition-all duration-200',
            activeMenu === item.path
              ? 'border-neutral-300 bg-neutral-100 text-foreground shadow-sm'
              : 'border-transparent text-neutral-700 hover:border-neutral-200 hover:bg-neutral-100 hover:text-foreground',
          ]"
        >
    ...
    <section class="flex min-h-screen flex-1 flex-col">
      <header class="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-border bg-white/90 px-6 backdrop-blur-sm">
        <div>
          <h2 class="text-[20px] font-semibold leading-[1.4] text-foreground">{{ pageTitle }}</h2>
          <p class="text-[12px] leading-[1.6] text-neutral-500">{{ pageSubtitle }}</p>
        </div>
        <div class="relative">
          <button
...
import { Cloud, FolderOpen, Trash2, Shield, CircleUserRound, ChevronDown, LogOut } from 'lucide-vue-next'
...
const menus = [
  { path: '/', label: '我的文件', icon: FolderOpen },
...
const pageTitle = computed(() => {
  if (route.path === '/recycle') return '回收站'
  if (route.path === '/admin') return '管理面板'
  return '我的文件'
})

const pageSubtitle = computed(() => {
  if (route.path === '/recycle') return '集中管理已删除文件'
  if (route.path === '/admin') return '更新用户会员状态'
