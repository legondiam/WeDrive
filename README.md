...
- **多级目录**：支持任意层级文件夹（`parent_id` 树形结构），根目录 `parent_id=0`，与常见网盘交互一致。
- **软删除与回收站**：文件删除进入回收站，可恢复，避免误删；列表接口只查未删除数据，逻辑清晰。
...
| 文件     | 上传、秒传、按目录列表、软删除、回收站列表与恢复 |
| 文件夹   | 创建多级目录（仅元数据），与文件统一展示 |
